<script lang="ts">
    ...
    // FIX 2 & 3: Deduplicate first, then convert to Array to sort safely
    let documentCategories = $derived(
        Array.from(new Set(documentDatas.map((d) => d._DocumentType)))
        .sort((a, b) => {
    ...
    <ol class="flex flex-col items-center gap-4">
        {#each documents as documentData (documentData._DocumentGU)}
            <li class="w-full max-w-3xl">
                <Button
                    href="{base}/documents/document?documentGU={documentData._DocumentGU}"
                    target="_blank"
                    variant="card"
                    size="lg"
